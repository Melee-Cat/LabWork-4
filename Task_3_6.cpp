...
public: // Публичная секция
    ...
    String operator+(const String& other) // Перегрузка оператора бинарного сложения
    {
        String plus;
        plus.length = length + other.length;
        delete[] plus.value;
        plus.value = new char[plus.length+1];
        for (int i=0; i<length; i++) {
            plus.value[i] = value[i];
        }
        for (int i=0; i<=other.length; i++) {
            plus.value[length+i] = other.value[i];
        }
        return plus;
    }
